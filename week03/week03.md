...
This week I have mainly focused on understanding encryption techniques and attacks concepts with some practical implementation. The main objective was to explore how block ciphers such as DES operate, how different modes of operation affect security, and how encryption can be applied in real-world scenarios.
...
A plaintext file of exactly 24 bytes was created This things has confirmed the file size was 24 bytes, which matches three DES blocks (8 bytes each).First encryption was performed and then Ciphertext was viewed. This confirmed the file size was 24 bytes, which matches three DES blocks (8 bytes each).
...
DES is a block cipher that processes fixed 8-byte blocks. Since the plaintext size was exactly 24 bytes, andthere is no need of padding as ECB mode encrypts each block independentally.
...
DES encryption was performed using both ECB and CBC modes with the same plaintext and a key.The ciphertext produced by ECB mode showed consistent block patterns because each block is encrypted independently. Overally, the ciphertext produced using CBC mode was completely different across all blocks. This things occurs because CBC mode uses an initialization vector (IV) and combines each plaintext block with the previous ciphertext block before encryption. As a result, even identical plaintext blocks produce different ciphertext outputs.
...
A random 64-bit key was generated using OpenSSL, This key was used to encrypt the README.md file using DES in ECB mode.The encrypted output was saved as README.bin. The file size confirmed that encryption was successful.
The encrypted file was then added to the Git repository and pushed to GitHub, demonstrating version control of encrypted data. This task shows how encryption can be integrated into real workflows and managed using Git.
